...
func (r *mutationResolver) NewChatRoom(ctx context.Context, input model.NewChatRoom, receiverID *int) (model.ChatRoom, error) {
	...
	if input.ChatRoomType.String() == "PRIVATE" {
		...
		if !isChatRoomExist {
			...
			_, err = crConn.Db.Exec("INSERT INTO members (chatroom_id, member_id, joined_at) VALUES ($1, $2, $3), ($1, $4, $3)", chatroom.ChatRoomID, input.CreatorID, time.Now(), receiverID)
			if err != nil {
				er.DebugPrintf(err)
				return model.ChatRoom{}, er.InternalServerError
			}
		...
		row := crConn.Db.QueryRow("INSERT INTO members (chatroom_id, member_id, joined_at) SELECT *FROM [INSERT INTO chatrooms (creator_id, chatroom_name, chatroom_type, created_at) VALUES ($1, $2, $3, $4) returning id, creator_id, created_at]", input.CreatorID, input.ChatRoomName, input.ChatRoomType, time.Now())
		err := row.Scan(&chatroom.ChatRoomID)
		if err != nil {
			er.DebugPrintf(err)
			return model.ChatRoom{}, er.InternalServerError
...
func (r *chatRoomResolver) Creator(ctx context.Context, obj *model.ChatRoom) (model.User, error) {
	...
	crConn := ctx.Value("crConn").(*dal.DbConnection)
	row, _ := crConn.Db.Query("SELECT id, username, first_name, last_name, email, contact, bio, profile_picture, users.created_at, users.updated_at FROM users, chatrooms WHERE chatrooms.id = $1 AND chatrooms.creator_id = users.id", obj.ChatRoomID)
	defer row.Close()
	for row.Next() {
...
func (r *chatRoomResolver) UpdateBy(ctx context.Context, obj *model.ChatRoom) (*model.User, error) {
	...
	var user model.User
	rows, _ := crConn.Db.Query("SELECT id, username, first_name, last_name, email, contact, bio, profile_picture, users.created_at FROM users WHERE id = $1", updateById)
	defer rows.Close()
	for rows.Next() {
